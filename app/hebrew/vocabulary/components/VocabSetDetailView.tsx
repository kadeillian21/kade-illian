...
interface VocabSetDetailViewProps {
  selectedSet: VocabSet;
  onBackToLibrary: () => void;
  onStartStudying: (group: VocabGroup, mode: FlashcardMode) => void;
  onStudyFullSet: (mode: FlashcardMode) => void;
}
...
export default function VocabSetDetailView({
  selectedSet,
  onBackToLibrary,
  onStartStudying,
  onStudyFullSet,
}: VocabSetDetailViewProps) {
      ...
      <div className="container py-12 px-4 sm:px-6 lg:px-8 mx-auto max-w-4xl">
        ...
        <div className="space-y-4 mb-8">
          {selectedSet.groups.map((group, groupIndex) => {
            const days = suggestStudyDays(group);
            return (
