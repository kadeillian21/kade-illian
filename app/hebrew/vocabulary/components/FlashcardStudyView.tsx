...
import { HebrewVocabWord, VocabGroup, VocabSet } from '../data/types';
...
const CATEGORY_EMOJIS: Record<string, string> = {
  ...
  'Consonants': 'א', 'Vowels': '◌ָ', 'Syllables': '🎯',
};
...
interface FlashcardStudyViewProps {
  ...
  currentIndex: number;
  currentCard: HebrewVocabWord | null;
  isFlipped: boolean;
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#f5f1e8] to-[#e8dcc8]">
      <div className="container py-12 px-4 sm:px-6 lg:px-8 mx-auto">
        <div className="max-w-2xl mx-auto">
          ...
          {/* Feedback Indicators */}
          {showCorrectFeedback && (
            <div className="fixed top-4 right-4 bg-green-500 text-white px-4 py-2 rounded-xl shadow-lg animate-fade-in z-50">
              ✓ Correct!
            </div>
          )}
          {showIncorrectFeedback && (
            <div className="fixed top-4 right-4 bg-orange-500 text-white px-4 py-2 rounded-xl shadow-lg animate-fade-in z-50">
              Keep practicing!
            </div>
          )}
